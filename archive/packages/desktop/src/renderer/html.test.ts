...
describe("electron desktop startup", () => {
  test("renderer shows a startup fallback while async resources are pending", async () => {
    const source = await Bun.file(join(dir, "index.tsx")).text()

    expect(source).toContain('data-component="desktop-startup-fallback"')
    expect(source).toContain("recordFatalRendererError")
    expect(source).toContain("startup resource")
  })
})
