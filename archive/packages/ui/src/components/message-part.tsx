...
// Read-aloud (TTS) playback is a singleton across the whole timeline: starting
// one clip stops any other, so at most one assistant reply ever plays at once.
let currentReadAloudAudio: HTMLAudioElement | undefined
const [readAloudActiveID, setReadAloudActiveID] = createSignal<string | undefined>()
const [readAloudLoadingID, setReadAloudLoadingID] = createSignal<string | undefined>()

function stopReadAloud() {
  if (currentReadAloudAudio) {
    currentReadAloudAudio.pause()
    currentReadAloudAudio.src = ""
    currentReadAloudAudio = undefined
  }
  setReadAloudActiveID(undefined)
}

// Play a synthesized clip through the shared singleton player, so manual and
// app-side auto read-aloud never overlap (starting one stops any other).
export function playReadAloudAudio(input: { messageID: string; audio: string; format: string }) {
  stopReadAloud()
  const mime = input.format === "wav" ? "audio/wav" : "audio/mpeg"
  const audio = new Audio(`data:${mime};base64,${input.audio}`)
  const clear = () => {
    if (currentReadAloudAudio === audio) stopReadAloud()
  }
  audio.addEventListener("ended", clear)
  audio.addEventListener("error", clear)
  currentReadAloudAudio = audio
  setReadAloudActiveID(input.messageID)
  void audio.play().catch(() => clear())
}

function ReadAloudButton(props: { messageID: string; text: string }) {
  const data = useData()
  const i18n = useI18n()
  const playing = () => readAloudActiveID() === props.messageID
  const loading = () => readAloudLoadingID() === props.messageID
  const label = () => (playing() ? i18n.t("ui.message.readAloud.stop") : i18n.t("ui.message.readAloud.play"))

  const onClick = async () => {
    const read = data.readAloud
    if (!read) return
    if (playing()) {
      stopReadAloud()
      return
    ...
    if (!content) return
    stopReadAloud()
    setReadAloudLoadingID(props.messageID)
    try {
      const result = await read({ messageID: props.messageID, text: content })
      // A newer click may have superseded this request while awaiting.
      if (readAloudLoadingID() !== props.messageID) return
      if (!result?.audio) return
      playReadAloudAudio({ messageID: props.messageID, audio: result.audio, format: result.format })
    } finally {
      if (readAloudLoadingID() === props.messageID) setReadAloudLoadingID(undefined)
    }
  }

  return (
    <Show when={data.readAloud}>
      <Tooltip value={label()} placement="top" gutter={4}>
        <IconButton
          icon={playing() ? "stop" : "speaker"}
          size="normal"
          variant="ghost"
          disabled={loading()}
          onMouseDown={(e) => e.preventDefault()}
        ...
        <Show when={showCopy()}>
          <div data-slot="text-part-copy-wrapper" data-interrupted={interrupted() ? "" : undefined}>
            ...
            <Show when={props.message.role === "assistant"}>
              <ReadAloudButton messageID={props.message.id} text={text()} />
            </Show>
            <Show when={meta()}>
