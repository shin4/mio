...
Practical reference for the current tool-migration state in `packages/agent`.
...
Tool tests should use the existing Effect helpers in `packages/agent/test/lib/effect.ts`:
