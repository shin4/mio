    ...
    const publishGlobal = (event: EventV2.Payload) =>
      Effect.sync(() => {
        GlobalBus.emit("event", {
          // Tag the directory so the renderer routes this like any directory
          // event (Bus.publish does the same at bus/index.ts). Without it the
          // frontend would treat the event as "global" and applyGlobalEvent
          // would silently ignore session/message events. Becomes load-bearing
          // the moment experimentalEventSystem is enabled.
