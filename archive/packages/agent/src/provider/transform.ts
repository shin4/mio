...
function hasOpenAIReasoningContent(options: unknown) {
  if (!options || typeof options !== "object") return false
  return ["openaiCompatible", "openai", "mimo"].some((key) => {
...
function filterMimoOpenAIEmptyAssistantMessages(msgs: ModelMessage[], model: Provider.Model): ModelMessage[] {
  if (model.providerID !== "mimo" || model.api.npm !== "@ai-sdk/openai") return msgs
  return msgs.filter((msg) => {
    if (msg.role !== "assistant") return true
    if (hasOpenAIReasoningContent(msg.providerOptions)) return true
    if (typeof msg.content === "string") return msg.content.trim().length > 0
    if (!Array.isArray(msg.content)) return true
    return msg.content.some((part) => {
      if (part.type === "text") return part.text.trim().length > 0
      if (part.type === "tool-call") return true
      return "providerOptions" in part && hasOpenAIReasoningContent(part.providerOptions)
    })
  })
}
...
export function message(msgs: ModelMessage[], model: Provider.Model, options: Record<string, unknown>) {
  ...
  msgs = filterMimoOpenAIEmptyAssistantMessages(msgs, model)
  if (
    (model.providerID === "anthropic" ||
...
export function maxOutputTokens(model: Provider.Model, outputTokenMax?: number): number {
  const cap = outputTokenMax ?? (model.providerID === "mimo" ? model.limit.output : OUTPUT_TOKEN_MAX)
