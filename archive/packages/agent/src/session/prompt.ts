...
function statSizeBytes(info: { readonly size?: number | bigint }) {
  if (typeof info.size === "bigint") return Number(info.size)
  return info.size
}
        ...
        if (part.type === "file") {
          ...
          switch (url.protocol) {
                  ...
                  return [
                    {
                      ...
                      type: "text",
                      synthetic: true,
                      text: oversizedMediaText({ filename: part.filename, mime: part.mime, encodedBytes }),
                    },
                  ]
                }
              }
            ...
            case "file:": {
              ...
              const mime =
                Option.isSome(stat) && stat.value.type === "Directory" ? "application/x-directory" : part.mime
              const rawBytes = Option.isSome(stat) ? statSizeBytes(stat.value) : undefined
              const encodedBytes = rawBytes === undefined ? undefined : projectedBase64Bytes(rawBytes)
              if (
                ...
                return [
                  ...(referenceContext ? [{ ...referenceContext, messageID: info.id, sessionID: input.sessionID }] : []),
                  {
                    ...
                    type: "text",
                    synthetic: true,
                    text: oversizedMediaText({ filename: part.filename, mime, encodedBytes }),
                  },
                ]
              }
