...
import { ToolRepair } from "./llm/tool-repair"
...
interface ProcessorContext extends Input {
  ...
  stormDetector: ToolRepair.StormDetector
}
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      const ctx: ProcessorContext = {
        ...
        reasoningMap: {},
        stormDetector: new ToolRepair.StormDetector(),
      }
      let aborted = false
      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        switch (value.type) {
          ...
          case "tool-call": {
            ...
            // ── Storm suppression (MiMo tool-repair stage 4) ───────────────
            const stormMsg = ctx.stormDetector.check(value.name, input)
            if (stormMsg) {
              log.warn("storm suppression triggered", {
                tool: value.name,
                sessionID: ctx.sessionID,
              })
              // Suppress the tool call and break the loop with a user-visible message
          ...
          case "step-finish": {
            ...
            // ── Cache hit measurement (MiMo prefix-cache observability) ──────
            {
              const rawUsage = value.usage ?? new Usage({})
              const readTokens = Math.max(0, rawUsage.cacheReadInputTokens ?? 0)
      ...
      const process = Effect.fn("SessionProcessor.process")(function* (streamInput: LLM.StreamInput) {
        ...
        return yield* Effect.gen(function* () {
          ...
          // Backstop: the LLM stream ended without a finish reason and without an
          // error (e.g. a provider closed the SSE without a terminal finish_reason).
          // cleanup() set time.completed but not finish; without a finish the prompt
          // loop never exits and the session stays "busy" (思考中) forever. Force-
          // complete the turn so the loop terminates and status reaches idle. This
          // is provider-agnostic defense-in-depth behind the per-protocol finishes.
