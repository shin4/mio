...
export function fromError(
  ...
  switch (true) {
    ...
    case e instanceof LLMError: {
      // The native LLM runtime surfaces a structured LLMError. Map it to the
      // same MessageV2 error shapes the AI-SDK path produces so retry,
      // context-overflow compaction, and auth surfacing all work.
      ...
      const statusCode =
        "status" in reason && typeof reason.status === "number" ? reason.status : http?.response?.status
      const responseBody = http?.body
      if (ProviderError.isOverflow(reason.message) || (responseBody && ProviderError.isOverflow(responseBody))) {
        return new ContextOverflowError({ message: reason.message, responseBody }, { cause: e }).toObject()
      }
      // Transport failures (connection reset/drop mid-request, including a
      // mid-stream reset) are transient and safe to retry, even though
      // TransportReason.retryable is false in the shared taxonomy. The session
      // processor now owns retries (the native request sets executor
      // maxRetries=0), so surface them as retryable or it gives up immediately.
      const isTransient = reason._tag === "Transport"
      return new APIError(
        {
          message: reason.message,
          statusCode,
          isRetryable: e.retryable || isTransient,
