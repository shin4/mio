...
function statusWithFetch(
  ...
  if (
    ...
    return { type: "unsupported", reason: "provider is not openai, opencode, anthropic, or mimo" }
        ...
        execute: (args: unknown, ctx) =>
          Effect.tryPromise({
            try: () => {
              ...
              // Tool-repair stage 1 (inverse): if the schema was flattened in
              // request.ts (deep / wide schemas get dot-notation), MiMo now
              // returns dot-notation arg keys. Re-nest before invoking the
              // executor, which still expects the original nested shape.
              const normalized =
                isRecord(args) && Object.keys(args).some((k) => k.includes("."))
                  ? ToolRepair.unflattenArgs(args)
                  : args
              return item.execute(normalized, {
