...
export const model = (input: Provider.Model | RequestInput, headers?: Record<string, string>) => {
  ...
  // mimo-desktop only routes through the MiMo cloud or Anthropic-compat endpoint.
  if (model.api.npm === "@ai-sdk/anthropic") return Anthropic.configure(options).model(model.api.id)
  const apiKey = "model" in input ? input.apiKey : undefined
  return MiMo.configure({
    apiKey,
    baseURL: url,
    protocol: model.api.npm === "@ai-sdk/anthropic" ? "anthropic" : "openai",
    // Forward the merged per-model headers (only Anthropic.configure got them
    // before); MiMo.Config extends RouteDefaultsInput which accepts headers.
...
export const request = (input: RequestInput) => {
  ...
  return LLM.request({
    ...
    // The session processor owns retries (it adds retry status events + resets
    // reasoning state between attempts), so opt the transport executor out of
    // its own retry layer to avoid double-retrying transient 5xx.
    http: { maxRetries: 0 },
  })
}
