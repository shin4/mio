...
export function fromRow(row: SessionRow): Info {
  ...
  return {
    ...
    workspaceID: (row.workspace_id ?? undefined) as any,
...
export const Event = {
  ...
  /** Emitted after each LLM step with prefix-cache hit statistics. */
  CacheMeasured: BusEvent.define(
    "session.cache.measured",
    Schema.Struct({
      sessionID: SessionID,
      turnId: Schema.String,
      /** Tokens served from cache (0 on first turn or cache miss). */
      readTokens: Schema.Int,
      /** Tokens that had to be processed (cache miss). */
  ...
  PrefixDrift: BusEvent.define(
    "session.cache.prefix.drift",
    Schema.Struct({
      sessionID: SessionID,
...
export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, AppInfo.projectConfigDir, "plans")
...
export interface Interface {
  ...
  /** Returns the locked immutable-region hash, or undefined if not yet set. */
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Three-region context: lock the immutable-region hash on the first request,
    // then on every subsequent request verify it has not drifted. Direct SQL
    // because these fields are session-internal metadata, not part of the
    // SyncEvent flow that propagates to other clients.
    const getImmutableHash = (sessionID: SessionID) =>
      db((d) => {
        const row = d
          .select({ immutable_hash: SessionTable.immutable_hash })
          .from(SessionTable)
          .where(eq(SessionTable.id, sessionID))
          .get()
        return row?.immutable_hash ?? undefined
      })

    const setImmutableHash = (input: { sessionID: SessionID; hash: string }) =>
      db((d) => {
        d.update(SessionTable)
          .set({ immutable_hash: input.hash })
