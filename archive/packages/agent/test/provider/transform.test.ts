...
describe("ProviderTransform.message - MiMo Pro media capability filtering", () => {
  const mimoProModel: Model = {
    ...
    cost: {
      input: 0.000003,
      output: 0.000006,
      cache: { read: 0.000000025, write: 0 },
    },
    limit: {
      context: 1_048_576,
      output: 8192,
  ...
  test("drops MiMo OpenAI assistant messages that become empty on send", () => {
    ...
    expect(ProviderTransform.message(msgs, mimoProModel, {})).toEqual([
      { role: "user", content: "First question" },
      { role: "user", content: "Follow-up" },
    ])
  })

  test("replaces unsupported media file parts with user-visible errors", () => {
    const msgs: ModelMessage[] = [
      {
        role: "user",
        content: [
          { type: "text", text: "read these" },
          { type: "file", mediaType: "image/png", data: "AA==", filename: "chart.png" },
          { type: "file", mediaType: "audio/mpeg", data: "AA==", filename: "voice.mp3" },
          { type: "file", mediaType: "video/mp4", data: "AA==", filename: "clip.mp4" },
          { type: "file", mediaType: "application/pdf", data: "AA==", filename: "spec.pdf" },
          { type: "file", mediaType: "text/plain", data: "hello", filename: "notes.txt" },
        ],
      },
    ]
    ...
    expect(ProviderTransform.message(msgs, mimoProModel, {})).toEqual([
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: 'ERROR: Cannot read "chart.png" (this model does not support image input). Inform the user.',
          },
          {
            type: "text",
            text: 'ERROR: Cannot read "voice.mp3" (this model does not support audio input). Inform the user.',
          },
          {
            type: "text",
          ...
          {
            type: "text",
            text: 'ERROR: Cannot read "spec.pdf" (this model does not support pdf input). Inform the user.',
          },
          { type: "file", mediaType: "text/plain", data: "hello", filename: "notes.txt" },
        ],
      },
...
describe("ProviderTransform.maxOutputTokens - MiMo", () => {
  const model = {
  ...
  test("defaults to the official model output limit", () => {
    expect(ProviderTransform.maxOutputTokens(model)).toBe(128_000)
  })

  test("honors explicit output token caps", () => {
