...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.MIO_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.MIO_TEST_HOME = prev
      }),
  )

describe("skill", () => {
  it.live("discovers skills from .mimo/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".mimo", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
...
`,
  ...
  it.live("registers customize-opencode as a hidden legacy alias of customize-mimo", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const skill = yield* Skill.Service
          const all = yield* skill.all()
          const canonical = all.find((s) => s.name === "customize-mimo")
          const alias = all.find((s) => s.name === "customize-opencode")
          // Both stay registered (invokable); only the alias is hidden from the palette.
          expect(canonical).toBeDefined()
          expect(alias).toBeDefined()
          expect(canonical!.hidden).toBeFalsy()
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".mimo", "skill", "dir-skill", "SKILL.md"),
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".mimo", "skill", "dir-skill"))
  ...
  it.live("discovers multiple skills from .mimo/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".mimo", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".mimo", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".mimo", "skill", "no-frontmatter", "SKILL.md"),
              `# No Frontmatter
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".mimo", "skill", "manual-skill", "SKILL.md"),
              `---
name: manual-skill
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".mimo", "skill", "opencode-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .mimo/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".mimo", "skill", "agent-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .mimo/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".mimo", "skills", "agent-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .mimo/skills directory.
---
