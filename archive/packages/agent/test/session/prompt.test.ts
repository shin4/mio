...
noLLMServer.instance(
  "replaces oversized data media attachments with synthetic errors",
  () =>
    Effect.gen(function* () {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      for (const attachment of [
        { mime: "image/png", filename: "large.png" },
        { mime: "audio/mpeg", filename: "large.mp3" },
        { mime: "video/mp4", filename: "large.mp4" },
      ]) {
        ...
        expect(msg.parts.some((part) => part.type === "file" && part.filename === attachment.filename)).toBe(false)
        expect(
          msg.parts.some(
            (part) =>
              part.type === "text" &&
              part.synthetic &&
              part.text.includes(attachment.filename) &&
              part.text.includes("MiMo") &&
              part.text.includes("50 MB"),
          ),
        ).toBe(true)
...
noLLMServer.instance(
  "rejects oversized local media before converting file bytes to Base64",
  () =>
    Effect.gen(function* () {
      const { directory: dir } = yield* TestInstance
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const session = yield* sessions.create({})
      const mediaPath = path.join(dir, "large.mp4")

      yield* writeText(mediaPath, "")
      yield* Effect.promise(() => truncateFile(mediaPath, 40_000_000))
      ...
      expect(msg.parts.some((part) => part.type === "file" && part.filename === "large.mp4")).toBe(false)
      expect(
        msg.parts.some(
          (part) =>
            part.type === "text" &&
            part.synthetic &&
            part.text.includes("large.mp4") &&
            part.text.includes("MiMo") &&
            part.text.includes("50 MB"),
        ),
      ).toBe(true)
