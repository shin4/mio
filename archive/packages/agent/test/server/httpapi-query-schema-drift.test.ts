...
describe("httpapi query schema drift", () => {
  // mimo-desktop routes on `directory` only; the legacy `workspace` query
  // param was removed with the control-plane workspace feature.
  ...
  it.live(
    "vcs diff accepts directory routing param",
    // git: true — /vcs/diff needs a real repo, otherwise it errors for reasons
    // unrelated to the query-schema drift this guards against.
