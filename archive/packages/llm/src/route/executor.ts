...
export interface Interface {
  readonly execute: (
    ...
    // Per-request retry budget. Defaults to MAX_RETRIES; callers that own
    // retries at a higher layer pass 0 to opt out of transport-level retries.
    maxRetries?: number,
...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      execute: (request, maxRetries) => retryStatusFailures(executeOnce(request), maxRetries ?? MAX_RETRIES),
    })
  }),
)
