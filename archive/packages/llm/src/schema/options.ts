...
export class HttpOptions extends Schema.Class<HttpOptions>("LLM.HttpOptions")({
  ...
  // Per-request override for the transport executor's retry budget. When unset
  // the executor uses its default. A caller that owns retries at a higher layer
  // (e.g. the session processor, which adds status events) sets this to 0.
...
export const mergeHttpOptions = (...items: ReadonlyArray<HttpOptions | undefined>): HttpOptions | undefined => {
  ...
  // Last explicit value wins (later items override earlier route defaults).
  const maxRetries = items.reduce<number | undefined>((acc, item) => item?.maxRetries ?? acc, undefined)
  if (!body && !headers && !query && maxRetries === undefined) return undefined
  return new HttpOptions({ body, headers, query, maxRetries })
}
