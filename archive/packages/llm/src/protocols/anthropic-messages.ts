...
interface ParserState {
  ...
  // Set once a terminal event (a message_delta finish, or a provider error) has
  // been emitted, so the onHalt fallback below does not double-emit a finish.
...
const onMessageDelta = (state: ParserState, event: AnthropicEvent): StepResult => {
  ...
  return [{ ...state, lifecycle, usage, finished: true }, events]
}

// onHalt fallback: if the stream ended (normal close) without a terminal
// message_delta — a truncated/proxy-cut Anthropic-compat response — no finish
// was ever emitted, so the processor would never set assistantMessage.finish
// and the session would hang "busy" forever. Emit a finish so the turn
// completes. (Mirrors openai-chat's finishEvents; transport errors take the
// catchCause path instead and never reach here.)
const finishEvents = (state: ParserState): ReadonlyArray<LLMEvent> => {
  if (state.finished) return NO_EVENTS
  const events: LLMEvent[] = []
  Lifecycle.finish(state.lifecycle, events, { reason: "stop", usage: state.usage })
  return events
}
...
const onError = (state: ParserState, event: AnthropicEvent): StepResult => [
  // Mark finished so the onHalt fallback does not append a spurious finish after
  // a provider error (the error is the terminal outcome of this stream).
  { ...state, finished: true },
