...
// Tool-repair stage 3: truncation repair.
// When finish_reason === "length" cuts a tool call mid-stream, JSON.parse fails.
// partial-json balances brackets / quotes and recovers what's there. The
// repaired call may have missing fields, but the executor surfaces those as
// normal validation errors — far better than dropping the whole call.
function recoverPartialJson(input: string): unknown | undefined {
  try {
  ...
  Effect.try({
    try: () => {
      try {
        ...
        const recovered = recoverPartialJson(input)
