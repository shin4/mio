...
export function Titlebar(props: { update?: TitlebarUpdate }) {
  ...
  // v1 cleanup: titlebar is always v2 now. Memo kept (always-true) so the
  // existing call sites compile without rewriting the conditional class lists
  // in this large component. Phase 6 of v1-cleanup will remove the references.
              ...
              >
                ...
                <Show when={!!params.dir}>
                  <IconButtonV2
                    ...
                    aria-label={language.t("sidebar.toggle.ariaLabel")}
                ...
                <div class="flex min-w-0 flex-1 flex-row items-center gap-1.5 overflow-hidden">
                  ...
                  {/* Portal target for the session file-search button (session-header.tsx). */}
                  <div id="opencode-titlebar-center" class="min-w-0 flex-1 flex items-center justify-center" />
                </div>
...
function TitlebarUpdatePill(props: { state: TitlebarUpdatePillState }) {
  return (
    <Show when={props.state.visible}>
      <button
        type="button"
        class="h-5 shrink-0 rounded-[27px] bg-[var(--v2-background-bg-layer-03)] px-2.5 text-[11px] font-[500] leading-4 tracking-[0.05px] text-[var(--v2-text-text-base)] disabled:opacity-60"
