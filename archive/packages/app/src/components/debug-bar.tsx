...
export function DebugBar() {
  ...
  const [state, setState] = createStore({
    cache: undefined as number | undefined,
  ...
  // Surface MiMo prefix-cache health: the latest hit rate and a running count
  // of prefix-drift events. These bus events aren't in the generated SDK Event
  // union (the SDK isn't regenerated in this fork), so cast at the boundary.
  const serverSDK = useServerSDK()
  onCleanup(
    serverSDK.event.listen((e) => {
      const detail = e.details as unknown as { type?: string; properties?: { hitRate?: number } }
      if (detail.type === "session.cache.measured") {
        const rate = Number(detail.properties?.hitRate)
        if (Number.isFinite(rate)) setState("cache", rate)
      } else if (detail.type === "session.cache.prefix.drift") {
        setState("drift", (value) => (value ?? 0) + 1)
      }
    }),
  )
  ...
  return (
    ...
    >
      <div class="grid grid-cols-5 gap-px font-mono">
        ...
        />
        <Cell
          label={language.t("debugBar.cache.label")}
          tip={language.t("debugBar.cache.tip")}
          value={state.cache === undefined ? na() : `${Math.round(state.cache * 100)}%`}
          bad={bad(state.cache, 0.7, true)}
          dim={state.cache === undefined}
        />
        <Cell
          label={language.t("debugBar.drift.label")}
          tip={language.t("debugBar.drift.tip")}
          value={state.drift === undefined ? na() : `${state.drift}`}
          bad={(state.drift ?? 0) > 0}
          dim={!state.drift}
        />
      </div>
