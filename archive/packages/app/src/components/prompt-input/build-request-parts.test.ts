...
describe("buildRequestParts", () => {
  ...
  test("forwards video and pdf attachments as file parts with their mime", () => {
    const result = buildRequestParts({
      prompt: [{ type: "text", content: "analyze these", start: 0, end: 13 }],
      context: [],
      images: [
        { type: "image", id: "img_1", filename: "a.png", mime: "image/png", dataUrl: "data:image/png;base64,AAA" },
      ],
      videos: [
        { type: "video", id: "vid_1", filename: "clip.mp4", mime: "video/mp4", dataUrl: "data:video/mp4;base64,VVV" },
      ],
      pdfs: [
        {
          type: "pdf",
          ...
          filename: "spec.pdf",
          mime: "application/pdf",
          dataUrl: "data:application/pdf;base64,PPP",
        },
      ],
      text: "analyze these",
      messageID: "msg_media",
    ...
    const media = result.requestParts.filter(
      (part): part is Extract<typeof part, { type: "file" }> => part.type === "file" && part.url.startsWith("data:"),
    )
    expect(media).toHaveLength(3)
    const nameFor = (mime: string) => media.find((part) => part.mime === mime)?.filename
    expect(nameFor("image/png")).toBe("a.png")
    expect(nameFor("video/mp4")).toBe("clip.mp4")
    expect(nameFor("application/pdf")).toBe("spec.pdf")
  })
