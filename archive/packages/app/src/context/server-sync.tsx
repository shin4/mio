...
type LiveDirectoryEventChildren = Pick<
  ReturnType<typeof createChildStoreManager>,
  "children" | "ensureChild" | "mark" | "vcsCache"
>

export function applyLiveDirectoryEvent(input: {
  directory: string
  event: { type: string; properties?: unknown }
  children: LiveDirectoryEventChildren
  ...
  const existing = input.children.children[key]
  const [store, setStore] = existing ?? input.children.ensureChild(input.directory)
  input.children.mark(input.directory)
  applyDirectoryEvent({
    event: input.event,
    ...
    vcsCache: input.children.vcsCache.get(key),
    loadLsp: input.loadLsp,
  })
  if (!existing) input.push(input.directory)
}
...
export function createServerSyncContext() {
  ...
  onMount(() => {
    // Start the live event stream WITHOUT gating on requestAnimationFrame.
    // rAF callbacks are suspended by Chromium whenever the renderer is
    // hidden/occluded — which is common right after a reload or when the window
    // sits behind another. If onMount runs while hidden, the rAF path leaves
    // serverSDK.event.start() un-invoked, the SSE stream never opens, NO live
    // events are ever applied, and every reply stays stuck on "思考中". A plain
    // timeout still fires while hidden, and start() is idempotent (guarded by
    // `started` in server-sdk.tsx), so this is safe to call eagerly.
  ...
  return {
    ...
    // A function (not a getter): the context return is spread via `{ ...sync }`,
    // which would snapshot a getter to its mount value. A function is copied by
    // reference, so it stays live. True once the global providers query has
    // resolved. Used by the onboarding gate to avoid a load-time flash.
    providersLoaded: () => !providerQuery.isLoading,
