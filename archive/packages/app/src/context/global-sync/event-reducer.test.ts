...
// Regression coverage for the "no reply / stuck 思考中" bug: a session running
// in an untracked directory had its live events dropped before they reached the
// store. server-sync.tsx now lazily creates the store and RE-APPLIES the
// dropped event. These tests prove that re-applying those events against a
// freshly-created (empty) store recovers the completed reply.
describe("applyDirectoryEvent — recovering dropped events on a fresh store", () => {
  const completedAssistant = (id: string, sessionID: string, completed: number) =>
    ({
      id,
      sessionID,
      role: "assistant",
  ...
  test("re-applied message.updated lands a completed assistant message (un-sticks pending)", () => {
    const [store, setStore] = createStore(baseState())

    applyDirectoryEvent({
      event: { type: "message.updated", properties: { info: completedAssistant("msg_a", "ses_1", 12345) } },
      store,
      setStore,
      push() {},
      directory: "/Users/shin",
      loadLsp() {},
    })

    expect(store.message.ses_1).toBeDefined()
    expect(store.message.ses_1.map((m) => m.id)).toEqual(["msg_a"])
    // time.completed must be a number — message-timeline's pending() treats a
    // missing/non-number completed as still-running ("思考中").
    expect((store.message.ses_1[0].time as { completed?: number }).completed).toBe(12345)
  })

  test("re-applied message.part.updated lands the reply text on a fresh store", () => {
    const [store, setStore] = createStore(baseState())
    const part = { ...textPart("prt_1", "ses_1", "msg_a"), text: "你的 workspace 下有多个项目" } as Part
    ...
    expect(store.part["msg_a"]).toBeDefined()
    expect((store.part["msg_a"][0] as { text: string }).text).toContain("多个项目")
  })

  test("re-applied session.status idle flips the session out of busy on a fresh store", () => {
    const [store, setStore] = createStore(baseState())
    ...
    expect(store.session_status.ses_1.type).toBe("idle")
  })
})
