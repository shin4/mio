...
export default function Page() {
  ...
  const sessionPanelWidth = createMemo(() => {
    ...
    if (desktopReviewOpen()) return `calc(100% - ${layout.review.width()}px)`
    return `calc(100% - ${layout.fileTree.width()}px)`
  })
  // Keep the conversation centered/readable now that it flexes in review mode.
  ...
  const line = (id: string) => {
    const text = draft(id)
      .map((part) => {
        if (part.type === "image" || part.type === "audio" || part.type === "video" || part.type === "pdf")
  ...
  const followupText = (item: FollowupDraft) => {
    const text = item.prompt
      .map((part) => {
        ...
        if (part.type === "video") return `[video:${part.filename}]`
        if (part.type === "pdf") return `[pdf:${part.filename}]`
