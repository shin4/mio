...
describe("isValidNewSessionDirectory", () => {
  test("rejects empty, root, and the home directory", () => {
    expect(isValidNewSessionDirectory("", "/Users/shin")).toBe(false)
    expect(isValidNewSessionDirectory(undefined, "/Users/shin")).toBe(false)
    expect(isValidNewSessionDirectory(null, "/Users/shin")).toBe(false)
    expect(isValidNewSessionDirectory("   ", "/Users/shin")).toBe(false)
    expect(isValidNewSessionDirectory("/", "/Users/shin")).toBe(false)
    expect(isValidNewSessionDirectory("/Users/shin", "/Users/shin")).toBe(false)
    // trailing-slash variants normalize to the same key as home
    expect(isValidNewSessionDirectory("/Users/shin/", "/Users/shin")).toBe(false)
  })

  test("accepts a real project directory", () => {
    expect(isValidNewSessionDirectory("/Users/shin/Developer/Side2/opencode", "/Users/shin")).toBe(true)
    expect(isValidNewSessionDirectory("/tmp/app", "/Users/shin")).toBe(true)
  })

  test("only rejects empty/root when home is unknown (early load)", () => {
    expect(isValidNewSessionDirectory("/Users/shin", "")).toBe(true)
    expect(isValidNewSessionDirectory("/Users/shin", undefined)).toBe(true)
    expect(isValidNewSessionDirectory("/tmp/app")).toBe(true)
    expect(isValidNewSessionDirectory("", undefined)).toBe(false)
    expect(isValidNewSessionDirectory("/", undefined)).toBe(false)
  })
})
