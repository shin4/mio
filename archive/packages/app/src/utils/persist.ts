...
function workspaceStorageName(prefix: string, dir: string) {
  const head = (dir.slice(0, 12) || "workspace").replace(/[^a-zA-Z0-9._-]/g, "-")
  const sum = checksum(dir) ?? "0"
  return `${prefix}.workspace.${head}.${sum}.dat`
}
...
function legacyWorkspaceStorage(dir: string) {
  ...
  result.add(workspaceStorageName(LEGACY_LOCAL_PREFIX, key))
  if (dir !== key) result.add(workspaceStorageName(LEGACY_LOCAL_PREFIX, dir))

  const drive = key.length >= 3 && key[1] === ":" && key[2] === "/"
  if (drive) result.add(workspaceStorageName(LEGACY_LOCAL_PREFIX, key.replaceAll("/", "\\")))
