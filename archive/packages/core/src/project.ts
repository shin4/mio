...
const CACHE_FILE = "mimo"
const LEGACY_CACHE_FILE = "opencode"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      ...
      return (yield* read(CACHE_FILE)) ?? (yield* read(LEGACY_CACHE_FILE))
    })
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, CACHE_FILE), input.id).pipe(Effect.ignore)
    })
