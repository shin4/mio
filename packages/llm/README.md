# MiMo-Code LLM Runtime

`@opencode-ai/llm` is kept under its compatibility package scope for now. It is
the schema-first LLM routing layer used by MiMo-Code's native request path.
...
not restored here. Additions should be driven by MiMo-Code runtime needs, not by
