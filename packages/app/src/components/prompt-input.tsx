...
import { agentDisplayLabel } from "@/utils/agent"
...
import { validateDictationAudio, type DictationAudioRejectionReason } from "@opencode-ai/core/dictation-audio"
...
// Media attachments (image/audio/video/pdf) live in the prompt array but are
// rendered as preview tiles, not as inline editor content — so they're
// excluded from DOM reconciliation and text-length math.
const isMediaPart = (part: ContentPart) =>
  part.type === "image" || part.type === "audio" || part.type === "video" || part.type === "pdf"
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const scrollCursorIntoView = () => {
    ...
    const length = promptLength(prompt.current().filter((part) => !isMediaPart(part)))
    if (cursor >= length) {
  ...
  const MEDIA_TYPES = new Set<MediaAttachmentPart["type"]>(["image", "audio", "video", "pdf"])
  const mediaAttachments = createMemo(() =>
    prompt
      .current()
      .filter((part): part is MediaAttachmentPart => MEDIA_TYPES.has(part.type as MediaAttachmentPart["type"])),
  )
  ...
  const [store, setStore] = createStore<{
    ...
    placeholder: number
    draggingType: "image" | "audio" | "video" | "pdf" | "@mention" | null
    mode: "normal" | "shell"
  ...
  const [dictationRemaining, setDictationRemaining] = createSignal(DICTATION_MAX_SECONDS)
  const [dictationLevel, setDictationLevel] = createSignal(0)
  let dictationCapture: DictationCapture | undefined
  let dictationTimer: ReturnType<typeof setTimeout> | undefined
  let dictationTick: ReturnType<typeof setInterval> | undefined
  let dictationCursor: number | undefined
  let dictationStopping = false

  const dictationTip = () => {
    if (dictationStatus() === "recording") {
      return language.t("prompt.dictation.stopWithCost", {
        time: readableDictationSeconds(dictationRemaining()),
      })
    }
    if (dictationStatus() === "transcribing") return language.t("prompt.dictation.transcribingWithModel")
    if (!ttsConfigured()) return language.t("prompt.dictation.unavailableWithCost")
  ...
  const showDictationRejection = (reason: DictationAudioRejectionReason) => {
    showToast({
      title: language.t("prompt.toast.dictationRejected.title"),
      description: language.t(
        reason === "too_short"
          ? "prompt.toast.dictationTooShort.description"
          : "prompt.toast.dictationNoSpeech.description",
      ),
    })
  }
  ...
  const clearDictationTimers = () => {
    ...
    if (dictationTick) clearInterval(dictationTick)
    dictationTimer = undefined
    dictationTick = undefined
  }

  const releaseDictationCapture = (capture: DictationCapture) => {
  ...
  const insertDictationText = (text: string) => {
    const transcript = text.trim()
    if (!transcript) {
      showDictationError("prompt.toast.dictationEmpty.description")
      return
    }

    const current = prompt.current()
    const media = mediaAttachments()
    const inline = current.filter((part) => !isMediaPart(part))
    const cursor = dictationCursor ?? prompt.cursor() ?? promptLength(inline)
    const hasStructuredInline = inline.some((part) => part.type !== "text")

    if (hasStructuredInline) {
      editorRef.focus()
      setCursorPosition(editorRef, cursor)
      addPart({ type: "text", content: transcript, start: 0, end: transcript.length })
      return
    }

    const next = insertTranscript(promptText(inline), transcript, cursor)
    prompt.set([{ type: "text", content: next.text, start: 0, end: next.text.length }, ...media], next.cursor)
    requestAnimationFrame(() => {
      editorRef.focus()
  ...
  function startDictation() {
    ...
    if (dictationStatus() === "transcribing") return
    if (!ttsConfigured()) {
      showToast({
        title: language.t("prompt.toast.dictationUnavailable.title"),
        description: language.t("prompt.toast.dictationUnavailable.description"),
      })
      return
    }
    if (!navigator.mediaDevices?.getUserMedia) {
      showDictationError("prompt.toast.dictationUnsupported.description")
      return
    }

    dictationCursor = currentCursor() ?? prompt.cursor() ?? promptLength(prompt.current().filter((part) => !isMediaPart(part)))
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        processor.onaudioprocess = (event) => {
          ...
          chunks.push(chunk)
          setDictationLevel((previous) => previous * 0.68 + dictationInputLevel(chunk) * 0.32)
        }
        ...
        setDictationRemaining(DICTATION_MAX_SECONDS)
        setDictationLevel(0)
        dictationTick = setInterval(() => {
          const elapsed = (Date.now() - (dictationCapture?.startedAt ?? Date.now())) / 1000
          setDictationRemaining(Math.max(0, DICTATION_MAX_SECONDS - elapsed))
        }, 250)
        dictationTimer = setTimeout(() => stopDictation(), DICTATION_MAX_SECONDS * 1000)
      })
      .catch(() => {
        dictationCursor = undefined
        setDictationLevel(0)
        showToast({
          title: language.t("prompt.toast.dictationPermission.title"),
          description: language.t("prompt.toast.dictationPermission.description"),
        })
      })
  }

  function stopDictation(submit = true) {
    const capture = dictationCapture
    if (!capture || dictationStopping) return
    dictationStopping = true
    dictationCapture = undefined
    releaseDictationCapture(capture)
    setDictationLevel(0)
    ...
    const validation = validateDictationAudio(samples, capture.sampleRate)
    if (!validation.ok) {
      dictationCursor = undefined
      dictationStopping = false
      setDictationStatus("idle")
      setDictationRemaining(DICTATION_MAX_SECONDS)
      showDictationRejection(validation.reason)
      return
    }
    const durationSeconds = Math.min(DICTATION_MAX_SECONDS, validation.durationSeconds)
    ...
    if (!http) {
      ...
      setDictationStatus("idle")
      setDictationRemaining(DICTATION_MAX_SECONDS)
      showDictationError("prompt.toast.dictationFailed.description")
      return
    }
    ...
    void transcribeDictation({
      ...
      .catch((error) => {
        const notConnected = error instanceof DictationError && error.code === "ProviderNotConnected"
        const rejected =
          error instanceof DictationError && (error.code === "too_short" || error.code === "no_speech")
        if (rejected) {
          showDictationRejection(error.code)
          return
        }
        showToast({
          title: language.t("prompt.toast.dictationFailed.title"),
          description: language.t(
            notConnected ? "prompt.toast.dictationUnavailable.description" : "prompt.toast.dictationFailed.description",
          ),
        })
      })
      .finally(() => {
  ...
  const cancelDictation = () => stopDictation(false)
  ...
  const handleCompositionEnd = () => {
    ...
    requestAnimationFrame(() => {
      if (composing()) return
      reconcile(prompt.current().filter((part) => !isMediaPart(part)))
    })
  }
  ...
  createEffect(
    on(
      ...
      (parts) => {
        if (composing()) return
        reconcile(parts.filter((part) => !isMediaPart(part)))
      },
    ),
  )
  ...
  const addPart = (part: ContentPart) => {
    if (isMediaPart(part)) return false
  ...
  const handleKeyDown = (event: KeyboardEvent) => {
    ...
    if (event.key === "Escape" && dictationStatus() === "recording") {
      cancelDictation()
      event.preventDefault()
  ...
  const agentControlState = createMemo<ComposerAgentControlState>(() => ({
    ...
    label: (value) => agentDisplayLabel(value, language.t),
    style: control(),
    onSelect: (value) => {
              ...
              >
                <div class="relative max-h-[180px] overflow-y-auto no-scrollbar" ref={(el) => (scrollRef = el)}>
                  <div
                    ...
                    classList={{
                      "select-text": true,
                      "min-h-[52px] w-full px-4 pt-4 pb-2 focus:outline-none whitespace-pre-wrap leading-5 text-[13px] font-[400] text-v2-text-text-base": true,
                  ...
                  <div
                    ...
                    class="absolute top-0 inset-x-0 px-4 pt-4 pointer-events-none whitespace-nowrap truncate leading-5 text-[13px] font-[400] text-v2-text-text-faint font-(family-name:--font-family-sans)"
              ...
              <div class="flex h-11 items-center px-2">
                <div class="flex min-w-0 flex-1 items-center gap-0">
                  ...
                  <Tooltip
                    ...
                    value={
                      !ttsConfigured()
                        ? language.t("prompt.tts.autoplay.unavailable")
                        : settings.tts.autoplay()
                          ? language.t("prompt.tts.autoplay.on")
                          : language.t("prompt.tts.autoplay.off")
                    }
                  >
                    <button
                      type="button"
                      data-action="prompt-tts-autoplay"
                      aria-pressed={ttsConfigured() && settings.tts.autoplay()}
                      aria-label={
                        !ttsConfigured()
                          ? language.t("prompt.tts.autoplay.unavailable")
                          : settings.tts.autoplay()
                      ...
                      onClick={() => ttsConfigured() && settings.tts.setAutoplay(!settings.tts.autoplay())}
                      class="flex size-7 shrink-0 items-center justify-center rounded-md transition-colors"
                      classList={{
                        "bg-[var(--mimo-accent-soft)] text-[var(--mimo-accent-text)]":
                          ttsConfigured() && settings.tts.autoplay(),
                        "text-v2-icon-icon-muted": !(ttsConfigured() && settings.tts.autoplay()),
                        "hover:bg-v2-overlay-simple-overlay-hover": ttsConfigured() && !settings.tts.autoplay(),
                      }}
                      style={ttsConfigured() ? buttons() : { ...buttons(), opacity: 0.4, cursor: "not-allowed" }}
                    >
                      <svg
                        width="18"
                        height="18"
                      ...
                      >
                        ...
                        <Show when={ttsConfigured() && settings.tts.autoplay()}>
                          <path class="mimo-wave" d="M14 9.8a3 3 0 0 1 0 4.4" />
                          <path class="mimo-wave mimo-wave-2" d="M16.4 7.6a6 6 0 0 1 0 8.8" />
                        </Show>
                      </svg>
                    </button>
                  </Tooltip>
                </div>
                <div data-action="prompt-primary-actions" class="flex shrink-0 items-center gap-2">
                  <Show when={dictationStatus() === "recording"}>
                    <div data-component="prompt-dictation-meter" aria-hidden="true">
                      <span data-slot="prompt-dictation-meter-bars">
                        <For each={dictationWaveBars(dictationLevel())}>
                          {(height) => (
                            <span
                              data-slot="prompt-dictation-meter-bar"
                              style={{
                                height: `${height}px`,
                              }}
                            />
                  ...
                  <Tooltip placement="top" gutter={4} value={dictationTip()}>
                    <IconButton
                      data-action="prompt-dictation"
                      type="button"
                      icon={dictationStatus() === "recording" ? "stop" : "mic"}
                      variant="ghost"
                      class="size-7 rounded-md p-[6px] text-v2-icon-icon-muted"
                      classList={{
                        "text-[var(--mimo-accent-text)] bg-[var(--mimo-accent-soft)]": dictationStatus() === "recording",
                      }}
                      style={ttsConfigured() ? buttons() : { ...buttons(), opacity: 0.4, cursor: "not-allowed" }}
                      onMouseDown={(e) => e.preventDefault()}
                      onClick={() => (dictationStatus() === "recording" ? stopDictation() : startDictation())}
                      disabled={store.mode !== "normal" || dictationStatus() === "transcribing"}
                      tabIndex={store.mode === "normal" ? undefined : -1}
                      aria-label={dictationTip()}
                    />
                  </Tooltip>
...
function ComposerPickerTrigger(props: ComponentProps<"button"> & { state: ComposerPickerTriggerState }) {
  ...
  return (
    <button
      ...
      type="button"
      class={`flex h-7 min-w-0 items-center gap-1.5 rounded px-2 text-[13px] font-[400] leading-5 tracking-[-0.04px] text-v2-text-text-faint transition-colors hover:bg-v2-overlay-simple-overlay-hover focus-visible:bg-v2-overlay-simple-overlay-hover focus-visible:outline-none ${local.state.class ?? ""}`}
...
function ComposerPickerMenuItem(props: { state: ComposerPickerItemState }) {
  return (
    <button
      type="button"
      class="flex h-7 w-full items-center gap-2 rounded px-3 text-left text-[13px] font-[400] leading-5 tracking-[-0.04px] text-v2-text-text-base hover:bg-v2-overlay-simple-overlay-hover focus-visible:bg-v2-overlay-simple-overlay-hover focus-visible:outline-none"
          ...
          <div class={`flex flex-col p-0.5 ${props.state.listClass ?? ""}`}>
            <div class="flex h-7 items-center gap-2 rounded px-3 text-v2-icon-icon-muted">
              ...
              <input
                ...
                class="h-7 min-w-0 flex-1 border-0 bg-transparent text-[13px] font-[400] leading-5 tracking-[-0.04px] text-v2-text-text-base outline-none placeholder:text-v2-text-text-faint"
    ...
    <div class="relative">
      ...
      <TooltipKeybind placement="top" gutter={4} title={props.state.title} keybind={props.state.keybind}>
        <Select
          ...
          valueClass="truncate pl-5 text-[13px] font-[400] leading-5 text-v2-text-text-faint"
        ...
        fallback={
          <TooltipKeybind placement="top" gutter={4} title={props.state.title} keybind={props.state.keybind}>
            <Button
              ...
              variant="ghost"
              size="normal"
              class="min-w-0 max-w-[220px] justify-start text-[13px] font-[400] leading-5 text-v2-text-text-faint group"
        ...
        <TooltipKeybind placement="top" gutter={4} title={props.state.title} keybind={props.state.keybind}>
          <ModelSelectorPopover
            ...
            triggerProps={{
              ...
              style: props.state.style,
              class:
                "min-w-0 max-w-[220px] justify-start text-[13px] font-[400] leading-5 text-v2-text-text-faint group",
              "data-action": "prompt-model",
            }}
