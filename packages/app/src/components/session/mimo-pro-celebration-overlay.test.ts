...
describe("MiMo Pro celebration overlay particles", () => {
  test("stays within the spec budget of 5-8 particles", () => {
    expect(mod.MIO_PRO_PARTICLES.length).toBeGreaterThanOrEqual(5)
    expect(mod.MIO_PRO_PARTICLES.length).toBeLessThanOrEqual(8)
  })

  test("derives every particle color from the fx tokens", () => {
    for (const particle of mod.MIO_PRO_PARTICLES) {
