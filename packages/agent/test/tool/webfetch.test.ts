    ...
    Effect.gen(function* () {
      ...
      yield* withFetch(
        (req) => {
          ...
          return new Response("retry ok", { status: 200, headers: { "content-type": "text/plain" } })
        },
        (url) =>
          Effect.gen(function* () {
            const result = yield* exec({ url: new URL("/challenge", url).toString(), format: "text" })
            expect(result.output).toBe("retry ok")
            expect(seen[1]).toBe("mimo-code")
          }),
      )
    }),
  )
