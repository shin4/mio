...
it.instance("ignores legacy OpenCode project config paths by default", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, "opencode.json"),
      JSON.stringify(schemaConfig({ model: "legacy/root" })),
    )
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".opencode", "agent", "legacy.md"),
      `---
model: legacy/model
---
Legacy agent prompt`,
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.mimo.managed.test",
          PayloadType: "ai.mimo.managed",
