...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "mimo")
      expect(yield* Effect.promise(() => Bun.file(cache).exists())).toBe(false)
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "mimo")
      const wrongCache = path.join(parentDir, ".git", "mimo")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "mimo")
      const cacheB = path.join(bareB, "mimo")
      const wrongCache = path.join(parentDir, ".git", "mimo")
