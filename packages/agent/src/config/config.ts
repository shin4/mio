...
function globalConfigFile() {
  const candidates = AppInfo.configFiles.map((file) => path.join(Global.Path.config, file))
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = AppInfo.configSchema
        const updated = text.replace(/^\s*\{/, `{\n  "$schema": "${AppInfo.configSchema}",`)
    ...
    const migrateGlobalConfig = Effect.fnUntraced(function* () {
      for (const item of globalConfigLegacyPairs) {
        const legacy = path.join(Global.Path.config, item.legacy)
        const next = path.join(Global.Path.config, item.next)
        if (!existsSync(legacy) || existsSync(next)) continue
        const text = yield* readConfigFile(legacy)
        if (text === undefined) continue
        yield* fs.writeWithDirs(next, text).pipe(Effect.catch(() => Effect.void))
      }
    })
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.MIMO_CONFIG && !Flag.MIMO_CONFIG_DIR && !Flag.MIMO_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: AppInfo.configSchema }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      for (const file of AppInfo.configFiles.toReversed()) {
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/${AppInfo.wellKnownConfigPath}`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = AppInfo.configSchema
