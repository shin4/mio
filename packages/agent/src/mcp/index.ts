...
import { Bus } from "@/bus"
// TuiEvent and `open` removed in mimo-desktop; replaced with stubs.
const TuiEvent = { ToastShow: { tag: "toast.show" } as any }
// open() previously launched the system browser to an OAuth URL. mimo-desktop
// has no in-app OAuth flow (single API-key auth) so this is a no-op that
// returns a Subprocess-shaped object so existing call sites compile.
const open = (_url: string): Promise<{ on(event: string, cb: (...args: any[]) => void): void }> =>
  Promise.resolve({ on: () => undefined })
  ...
  Effect.gen(function* () {
    ...
    const connectTransport = (transport: Transport, timeout: number) =>
      Effect.acquireUseRelease(
        ...
        (t) =>
          Effect.tryPromise({
            try: () => {
              const client = new Client({ name: "mio", version: InstallationVersion })
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          ...(cmd === "opencode" || cmd === "mimo" ? { BUN_BE_BUN: "1" } : {}),
          ...mcp.environment,
        },
      })
