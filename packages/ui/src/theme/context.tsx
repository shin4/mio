...
const STORAGE_KEYS = {
  THEME_ID: "mimo-theme-id",
  COLOR_SCHEME: "mimo-color-scheme",
  THEME_CSS_LIGHT: "mimo-theme-css-light",
  THEME_CSS_DARK: "mimo-theme-css-dark",
} as const
...
function read(key: string, legacyKey?: string) {
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { defaultTheme?: string; onThemeApplied?: (theme: DesktopTheme, mode: "light" | "dark") => void }) => {
    const themeId = normalize(read(STORAGE_KEYS.THEME_ID, LEGACY_STORAGE_KEYS.THEME_ID) ?? props.defaultTheme) ?? "oc-2"
    const colorScheme = (read(STORAGE_KEYS.COLOR_SCHEME, LEGACY_STORAGE_KEYS.COLOR_SCHEME) as ColorScheme | null) ?? "system"
    ...
    onMount(() => {
      ...
      const rawTheme = read(STORAGE_KEYS.THEME_ID, LEGACY_STORAGE_KEYS.THEME_ID)
      const savedTheme = normalize(rawTheme ?? props.defaultTheme) ?? "oc-2"
      const savedScheme = (read(STORAGE_KEYS.COLOR_SCHEME, LEGACY_STORAGE_KEYS.COLOR_SCHEME) as ColorScheme | null) ?? "system"
