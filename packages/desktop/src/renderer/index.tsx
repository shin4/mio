...
render(() => {
  ...
  // Restore the location the user was last on so a reload stays put instead of
  // dropping back to the home route (the in-memory router has no URL to fall
  // back on). Created once the route resolves and reused for the router's life.
  const [lastRoute] = createResource(loadPersistedRoute)
  const startupResources = createMemo(() => ({
    defaultServer: defaultServer.loading,
    lastRoute: lastRoute.loading,
    locale: locale.loading,
    sidecar: sidecar.loading,
    windowConfig: windowConfig.loading,
    windowCount: windowCount.loading,
  }))
  const pendingStartupResources = () =>
    Object.entries(startupResources())
      .filter((entry) => entry[1])
      .map((entry) => entry[0])
  const startupReady = () => pendingStartupResources().length === 0
  ...
  onMount(() => {
    ...
    const startupTimer = window.setTimeout(() => {
      const pending = pendingStartupResources()
  ...
  return (
    <PlatformProvider value={platform}>
      <AppBaseProviders locale={locale.latest}>
        ...
        >
          {(_) => {
            const history = attachRoutePersistence(createMemoryHistory(), lastRoute.latest)
            return (
              <AppInterface
                defaultServer={defaultServer.latest ?? ServerConnection.Key.make("sidecar")}
                servers={servers()}
                router={(routerProps) => <MemoryRouter {...routerProps} history={history} />}
              >
                <Inner />
              </AppInterface>
            )
