...
import { isPetWindowOpen } from "./pet-window"
...
export function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "MiMo-Code window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("MiMo-Code is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
...
export function allowRendererPermissions(win: BrowserWindow) {
  ...
  win.webContents.session.setPermissionCheckHandler((webContents, permission, requestingOrigin, details) => {
    if (!isAllowedRendererPermission(permission, details)) return false
    if (webContents && webContents.id !== win.webContents.id) return false
    return isTrustedRendererUrl(details.requestingUrl) || isTrustedRendererUrl(requestingOrigin)
  })
}

function isAllowedRendererPermission(permission: string, details: unknown) {
  ...
  return mediaDetails.mediaTypes?.includes("audio") === true || mediaDetails.mediaType === "audio"
}
